...
const {convertTotal, convertAverage, convertWorkingHours, mergeWorkingHours, convertAllWorkingHours, mergeAllWorkingHours, getDaysOff} = require("../services/historyService")

class historyController{
    showTotalWorkingHours(req, res){
        let { id, month, year, user_id } = req.body;
        month +=1;
        ...
        AND working_seats.user_id=${user_id}
    ...
    showAverageWorkingHours(req, res){
        let { id, month, year, user_id } = req.body;
        month +=1;
        let q = `SELECT history_test.id, history_test.updated_at as time, history_test.status 
        FROM history_test, working_seats 
        ...
        db.query(q, (err, result)=>{
            ...
            let convertedData = convertWorkingHours(result)
            let mergedData = mergeWorkingHours(convertedData)
            let finalData = convertAverage(mergedData)
            return res.status(200).json(finalData)
        })
    }
